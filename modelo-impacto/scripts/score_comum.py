...
`modelo-impacto/` (desenho pareado, trajetória de pixel, anéis, footprint OSM, LST) e por
`src/sentinela/` (classificador), e monta três camadas — medição, explicação, projeção.

Ver `modelo-impacto/README.md` para o desenho e as razões.
...
DIR_IMPACTO = RAIZ / "modelo-impacto"
DIR_RESULTADOS = DIR_IMPACTO / "raw" / "controles-rf"
DIR_PROCESSED = DIR_IMPACTO / "processed"
DIR_SAIDA = RAIZ / "modelo-impacto" / "outputs"
DIR_FIGURAS = RAIZ / "modelo-impacto" / "reports" / "figuras"

SEED = 42
N_ANOS_PONTA = 2  # mesma convenção do passo 12 de modelo-impacto
...
def placebo_da_estatistica() -> str:
    ...
    trajetória ordenada e persistente (passo 12 de `modelo-impacto`). O passo 19 aplicou
