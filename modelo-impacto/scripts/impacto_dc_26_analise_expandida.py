...
Rode com:
    ...
    python modelo-impacto/scripts/impacto_dc_26_analise_expandida.py --fase footprints
...
def mascara_footprint_do_campus(site: str, ref: Path) -> np.ndarray | None:
    """Footprint do campus, do passo 13 (originais) ou do passo 26 (novos).

    Pública porque o passo 40 precisa da mesma máscara para excluir o prédio do conjunto de
    risco — e tem de ser exatamente a mesma, senão os dois passos excluiriam áreas diferentes
    e suas contagens deixariam de ser comparáveis. Era privada (`_mascara_footprint`), e o
    passo 40 chamava este nome público antes de ele existir.
    """
...
def fase_analise() -> None:
    ...
    for _, r in pares.iterrows():
        ...
        for tipo, pid, lat, lon in (("tratamento", r.site_id, r.lat, r.lon),
            ...
            m_fp = mascara_footprint_do_campus(r.site_id, ref) if tipo == "tratamento" else None
