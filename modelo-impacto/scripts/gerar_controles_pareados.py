...
`modelo-impacto/processed/consolidado_facilities.csv`, `origem_lista == "datacentermap_novo"`)
...
`modelo-impacto/README.md`). Suas coordenadas SÃO usadas para enriquecer a lista de
...
Uso:
    .venv\\Scripts\\python.exe modelo-impacto\\scripts\\gerar_controles_pareados.py [--force]
...
FACILITIES_PATH = REPO_ROOT / "modelo-impacto" / "processed" / "consolidado_facilities.csv"

OUT_DIR = REPO_ROOT / "modelo-impacto" / "raw" / "controles"
...
USER_AGENT = (
    "sentinela-verde-mba-modelo-impacto/1.0 "
...
Gerado por `modelo-impacto/scripts/gerar_controles_pareados.py` em
...
daqui). Ver `modelo-impacto/README.md` para o contexto geral deste apoio.
...
- **Decisão explícita, não esquecimento**: os 3 sites novos reconciliados a partir do scraping do
  ...
  RT-One Uberlândia/MG — `modelo-impacto/processed/consolidado_facilities.csv`,
