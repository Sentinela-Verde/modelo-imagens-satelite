# O boletim de impacto — a camada de medição desta pasta

Criada em **2026-09-10** como pasta própria (`modelo-impacto-score/`) e fundida aqui em
**2026-09-13**. Os scripts são os `scripts/score_*.py`; as saídas, `outputs/`.

Não é o classificador de cobertura do solo (`src/sentinela/`) nem a coleta de dados externos
(a outra metade desta pasta, ver `README.md`) — **consome os artefatos das duas** e responde a
uma pergunta diferente: *qual foi o impacto territorial de cada data center, e ele é predizível?*

Tem prazo próprio e **não segue o cronograma do classificador** — ver `CLAUDE.md` na raiz.
...
**Isto não contradiz o achado greenfield/brownfield do passo 15 de `modelo-impacto`.** São
...
É o que o próprio README de `modelo-impacto` já antecipava: *"a leitura defensável é a do
...
```
python modelo-impacto/scripts/score_03_projecao.py --pct-ja-construida 15
...
- **Temperatura.** Medida **duas vezes** e reportada nas duas. Com Landsat de 30 m no anel — a
  ...
  `modelo-impacto/scripts/impacto_dc_23_lst_landsat.py` (e o passo 16, superado, que motivou
  a troca de sensor).
...
│   ├── score_01_boletim.py      # camada 1 — medição
│   ├── score_02_explicacao.py   # camada 2 — explicação (LOOCV + permutação)
│   └── score_03_projecao.py     # camada 3 — projeção por classe de referência
...
|---|---|---|
| `modelo-impacto/processed/` | `consolidado_impacto_painel.csv` | painel 204×51, 15 pares, 2013–2025 |
| `modelo-impacto/raw/controles-rf/` | `footprint_vs_anel.csv` | conversão por zona (footprint, 0–0,5, 0,5–1, 1–2 km) |
